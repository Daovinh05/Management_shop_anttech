...
<body>
    <div class="wrap">
        <div class="card">
            ...
            <form id="brandImportForm" method="POST" action="#" enctype="multipart/form-data">
                ...
                <div class="form-actions">
                    <a href="<?php echo BASE_URL; ?>Thuonghieu/danhsach" class="btn btn-secondary">
                        <i class="fa-solid fa-arrow-left"></i> Quay lại
                    </a>
                    <button type="submit" id="importBrandBtn" name="btnUpload" class="btn btn-primary" style="min-width: 150px;">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('brandImportForm');
            const submitBtn = document.getElementById('importBrandBtn');
            const resultBox = document.getElementById('apiImportResult');
            const summary = document.getElementById('apiImportSummary');

            if (!form || !submitBtn || !resultBox || !summary) {
                return;
            }
            ...
            form.addEventListener('submit', function(event) {
                ...
                fetch(BASE_URL + 'Api/Thuonghieu/import', {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(function(result) {
                    ...
                    if (!result.ok || !data.success) {
                        ...
                        const failedPreview = failedRows.slice(0, 5).map(function(item) {
                            return 'Dòng ' + item.row + ' (' + (item.ma_thuong_hieu || 'N/A') + '): ' + item.reason;
                        }).join('\n');

                        throw new Error((data.message || ('Import thất bại (HTTP ' + result.status + ')')) + (failedPreview ? ('\n' + failedPreview) : ''));
                    }

                    alert('Import thương hiệu thành công qua REST API.');
                })
